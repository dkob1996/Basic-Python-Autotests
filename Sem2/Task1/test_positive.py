...
def test_step1(make_folders, clear_folders, make_files):
    ...
    try:
        res = []
        res.append(checkout(f'cd {data["folder_in"]}; 7zz a -t{data["arc_type"]} {data["folder_out1"]}/arx2', positive_result()))
        log_step_info(STEP,res)
        res.append(checkout(f'ls {data["folder_out1"]}', f'arx2.{data["arc_type"]}'))
        log_step_info(STEP,res)
...
def test_step2(clear_folders, make_files): 
    ...
    try:
        ...
        res.append(checkout(f'cd {data["folder_in"]}; 7zz a -t{data["arc_type"]} {data["folder_out1"]}/arx2', positive_result()))
        log_step_info(STEP,res)
        res.append(checkout(f'cd {data["folder_out1"]}; 7zz e arx2.{data["arc_type"]} -o{data["folder_out2"]} -y', positive_result()))
        log_step_info(STEP,res)
...
def test_step3(make_folders, clear_folders, make_files):
    STEP = 3
    try:
        res = checkout (f'cd {data["folder_out1"]}; 7zz t arx2.{data["arc_type"]}', positive_result())
        log_step_info(STEP,res)
...
def test_step4(make_folders, clear_folders, make_files):
    ...
    try:
        ...
        res.append(checkout(f'cd {data["folder_in"]}; 7zz a -t{data["arc_type"]} {data["folder_out1"]}/arx2', positive_result()))
        log_step_info(STEP,res)
        res.append(checkout(f'cd {data["folder_out1"]}; 7zz u arx2.{data["arc_type"]}', positive_result()))
        log_step_info(STEP,res)
        assert all(res), error_message(STEP)
    except AssertionError as e:
        log_exception(STEP, e)
...
def test_step5(make_folders, clear_folders, make_files):
    ...
    try:
        ...
        res.append(checkout(f'cd {data["folder_in"]}; 7zz a -t{data["arc_type"]} {data["folder_out1"]}/arx2', positive_result()))
        log_step_info(STEP,res)
        res.append(checkout(f'cd {data["folder_out1"]}; 7zz d arx2.{data["arc_type"]}', positive_result()))
        log_step_info(STEP,res)
        assert all(res), error_message(STEP)
    except AssertionError as e:
        log_exception(STEP, e)
...
def test_step6(make_folders, clear_folders, make_files): 
    ...
    try:
        ...
        log_step_info(STEP,res)
        for item in make_files:
            res.append(checkout (f'cd {data["folder_out1"]}; 7zz l arx2.{data["arc_type"]}', item))
            log_step_info(STEP,res)
        assert all(res), error_message(STEP)
    except AssertionError as e:
       log_exception(STEP, e)
...
def test_step7(clear_folders, make_files, make_subfolder):
    ...
    try:
        ...
        log_step_info(STEP,res)
        # разархивация
        res.append(checkout (f'cd {data["folder_out1"]}; 7zz x arx2.{data["arc_type"]} -o{data["folder_out3"]} -y', positive_result()))
        log_step_info(STEP,res)
...
def test_step8(make_folders, clear_folders, make_files):
    ...
    try:
        ...
        res.append(checkout(f'cd {data["folder_out1"]}; 7zz h arx2.{data["arc_type"]}', positive_result()))
        log_step_info(STEP, res)
        ## считаем хэш через 7z
        actual_hash = checkhash(f'cd {data["folder_out1"]}; 7zz h arx2.{data["arc_type"]}')
        log_step_info(STEP, actual_hash)
        ## считаем хэш через аналог crc32 для macos - cksum
        expected_hash = calculate_crc32c(os.path.join(data["folder_out1"], f'arx2.{data["arc_type"]}')).upper()
        log_step_info(STEP, expected_hash)
        ## сравниваем хэши
        res.append(checkout(f'cd {data["folder_out1"]}; 7zz h arx2.{data["arc_type"]}', expected_hash))
        log_step_info(STEP,res)
